...
async function updateEnterprise(id: string, { values }: EnterpriseSubmitData) {
  const { TIN, name, PSRN, address, archive, contacts } = values;
  const body: EnterprisePutBody = {
    name,
    attributes: {
      addr: address,
      inn: TIN,
  ...
  await http.put(`/api/enterprises/${id}`, body, {
    onSuccess: () => notify.success('Информация о компании успешно обновлена'),
    onError: () => notify.error('Не удалось обновить информацию о компании'),
  });
}
